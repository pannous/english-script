...
# iseq_data_to_ary(rb_iseq_t *iseq) : LATER load directly, no ary array
# iseq gem needs array, we rather load binary dump
# iseq_marshal_load NOWHERE FOUND, so start:
# File.write("out.dump", Marshal.dump(seq.to_a))
# arr = Marshal.load(File.read("out.dump"))
# seq=RubyVM::InstructionSequence.compile("x>3") OK! todo inject or eval in context! ext.c: iseq_set_parent
...
# printf = host_module.functions.add("printf", [chars], LLVM::Int32Ty)
    ...
    show_version = host_module.functions.add("ruby_show_version",[],LLVM::Int32Ty);
...
# `COMPILER_ARGS=llvm-config --libs core jit native --cxxflags --ldflags`
    puts "LINKING!!!"
    `clang++ $COMPILER_ARGS ./build/libmruby.a  ./build/main.o -o ./build/main` # or any other gcc!!
    puts "EXECUTING FILE!!!"
    system("./build/main")
# system("./target/main&")
